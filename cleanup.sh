...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --purge-all)          PURGE_COGNITIVE=true; PURGE_APIM=true; shift ;;
        ...
        -h|--help)
            ...
            echo "  --purge-all             Purge all soft-deleted resources (cognitive + APIM)"
...
if ! az group show --name "$RESOURCE_GROUP_NAME" --output none 2>/dev/null; then
    ...
    if [[ "$PURGE_COGNITIVE" == "true" || "$PURGE_APIM" == "true" ]]; then
        echo ""
...
if [[ "$PURGE_COGNITIVE" == "true" ]]; then
    ...
    if [[ "$count" -gt 0 ]]; then
        ...
        echo "$deleted" | jq -c '.[]' | while read -r item; do
            ...
            if az cognitiveservices account purge --name "$name" --location "$location" --resource-group "$RESOURCE_GROUP_NAME" 2>/dev/null \
               || az rest --method DELETE --url "https://management.azure.com/subscriptions/$(az account show --query id -o tsv)/providers/Microsoft.CognitiveServices/locations/$location/resourceGroups/$RESOURCE_GROUP_NAME/deletedAccounts/$name?api-version=2021-10-01" 2>/dev/null; then
                echo "   ✅ Purged '$name'"
            else
